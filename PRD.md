...
- **Late-Game Operation opportunity dedup**: when rolling the d8 outcome table (on a successful Late-Game Scout), re-roll if the resulting type is already completed *or* already sitting unexecuted in `availableLateGameOps`. This is broader than the PDF's literal "already been successfully executed" wording — a deliberate deviation, worth a one-line comment at the reroll site pointing back to this decision.
